        ...
        try:
            ...
            with transaction, force_interactive_oauth(), dashboard_oauth_flow(flow):
                ...
                try:
                    ...
                    try:
                        oauth_callback_timeout = float(oauth_cfg.get("timeout", 300) or 300)
                    except (TypeError, ValueError):
                        oauth_callback_timeout = 300.0
                    tools = _probe_single_server(
                        flow.server_name,
                        cfg,
                        connect_timeout=max(float(cfg.get("connect_timeout", 0) or 0), oauth_callback_timeout + 15.0, 315.0),
                    )
